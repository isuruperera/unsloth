...
try:
    ...
    from core.inference.llama_cpp import LlamaCppBackend, SamplingParams
...
async def load_model(
    ...
    try:
        ...
        # trust_remote_code is never taken from the caller's request; it is
        # derived solely from the server-side model defaults YAML so a
        # client cannot enable remote code execution on load.
        ...
        if trust_remote_code:
            logger.info(
                f"Model '{config.display_name}' is configured to trust remote code "
                f"per its model defaults YAML."
            )
